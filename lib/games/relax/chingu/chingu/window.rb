...
module Chingu
  ...
  # - Sets a global variable $chingu_window = self, which is then used throughout Chingu
  ...
  class Window < Gosu::Window
    ...
    def initialize(width = 800, height = 600, fullscreen = false, update_interval = 16.666666)
      raise "Cannot create a new #{self.class} before the old one has been closed" if $chingu_window

      fullscreen ||= ARGV.include?("--fullscreen")
      $chingu_window = super(width, height, fullscreen, update_interval)
    ...
    # Chingu solves this with the $chingu_window.cursor = [true|false] accessor
    #
    def needs_cursor?
      @cursor
    end
    ...
    # ... Falls back to returning $chingu_window
    #
    ...
    #
    # Frames per second, access with $chingu_window.fps or $chingu_window.framerate
    #
    def fps
      @fps_counter.fps
    end
    ...
    def close
      ...
      # Clear out all assets, tied to this $chingu_window, so that a new instance can create more.
