...
module Chingu
  ...
  class Viewport
    ...
    def initialize(options = {})
      ...
      @game_area = Chingu::Rect.new(options[:game_area] || [@x, @y, $chingu_window.width, $chingu_window.height])
    end
    ...
    def center_around(object)
      self.x = object.x - $chingu_window.width / 2
      self.y = object.y - $chingu_window.height / 2
    end
    ...
    def inside?(object, y = nil)
      ...
      x >= @x && x <= (@x + $chingu_window.width) &&
      y >= @y && y <= (@y + $chingu_window.height)
    end
    ...
    def x=(x)
      ...
      if @game_area
        ...
        @x = @game_area.width-$chingu_window.width   if @x > @game_area.width-$chingu_window.width
      end
    end
    ...
    def y=(y)
      ...
      if @game_area
        ...
        @y = @game_area.height-$chingu_window.height   if @y > @game_area.height-$chingu_window.height
      end
    end
    ...
    #
    def apply(&block)
      $chingu_window.translate(-@x.to_i, -@y.to_i, &block)
    end
