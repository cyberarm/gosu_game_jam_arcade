...
module Chingu
  ...
  class GameState
    ...
    def initialize(options = {})
      ...
      if defined?($chingu_window) && $chingu_window.respond_to?(:game_state_manager)

        # Since we place the init of previous_game_state here, game states can use it even
        # in initialize() if they call super first.
        @previous_game_state = $chingu_window.game_state_manager.current_game_state

        $chingu_window.game_state_manager.inside_state = self
      end
