...
module Chingu
  module GameStates
    ...
    #
    # requires the global $chingu_window set to the instance of Gosu::Window (automaticly handled if you use Chingu::Window)
    #
    ...
    class Edit < Chingu::GameState
      ...
      def initialize(options = {})
        ...
        #
        @original_cursor = $chingu_window.cursor
        $chingu_window.cursor = true
...
END_OF_STRING
      ...
      def draw_grid
        ...
        (start_x .. $chingu_window.width).step(@grid.first).each do |x|
          $chingu_window.draw_line(x, 1, @grid_color, x, $chingu_window.height, @grid_color, 0, :additive)
        end
        (start_y .. $chingu_window.height).step(@grid.last).each do |y|
          $chingu_window.draw_line(1, y, @grid_color, $chingu_window.width, y, @grid_color, 0, :additive)
        end
      ...
      def update
        ...
        if @left_mouse_button && @selected_game_object
          ...
          if defined?(self.previous_game_state.viewport)
            self.previous_game_state.viewport.x = @left_mouse_click_at[0] - $chingu_window.mouse_x
            self.previous_game_state.viewport.y = @left_mouse_click_at[1] - $chingu_window.mouse_y
          end
        end

        if inside_window?($chingu_window.mouse_x, $chingu_window.mouse_y)
          scroll_right  if $chingu_window.mouse_x > $chingu_window.width - @scroll_border_thickness
          scroll_left   if $chingu_window.mouse_x < @scroll_border_thickness
          scroll_up     if $chingu_window.mouse_y < @scroll_border_thickness
          scroll_down   if $chingu_window.mouse_y > $chingu_window.height - @scroll_border_thickness
        end
      end
      ...
      def draw
        ...
        # Draw an edit HUD
        #
        $chingu_window.draw_quad(  0,0,@hud_color, $chingu_window.width,0,@hud_color,
                            $chingu_window.width,@hud_height,@hud_color,0,@hud_height,@hud_color)
        ...
        @cursor_game_object.draw_at($chingu_window.mouse_x, $chingu_window.mouse_y)   if @cursor_game_object
      end
      ...
      def left_mouse_button
        ...
        if defined?(self.previous_game_state.viewport)
          @left_mouse_click_at = [self.previous_game_state.viewport.x + $chingu_window.mouse_x, self.previous_game_state.viewport.y + $chingu_window.mouse_y]
        else
          @left_mouse_click_at = [$chingu_window.mouse_x, $chingu_window.mouse_y]
        end
        ...
        @cursor_game_object = game_object_icon_at($chingu_window.mouse_x, $chingu_window.mouse_y)
      ...
      def empty_area_at_cursor
        ...
        game_object_icon_at($chingu_window.mouse_x, $chingu_window.mouse_y) == nil
      end
      ...
      def finalize
        ...
        $chingu_window.cursor = @original_cursor
      end
      ...
      def page_up
        self.previous_game_state.viewport.y -= $chingu_window.height if defined?(self.previous_game_state.viewport)
      end
      def page_down
        self.previous_game_state.viewport.y += $chingu_window.height if defined?(self.previous_game_state.viewport)
      end
