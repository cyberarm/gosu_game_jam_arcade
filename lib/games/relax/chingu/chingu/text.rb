...
module Chingu
  ...
  # Pure Gosu:
  #   @font = Gosu::Font.new($chingu_window, "verdana", 30)
  ...
  class Text < Chingu::GameObject
    ...
    def initialize(text, options = {})
      ...
      @gosu_font = Gosu::Font.new(@size, name: @font)
    ...
    def create_image
      if @max_width
        @image = Gosu::Image.from_text($chingu_window, @text, @font, @size, @line_spacing, @max_width, @align)
      else
        @image = Gosu::Image.from_text($chingu_window, @text, @font, @size)
      end
    end
  end
end
