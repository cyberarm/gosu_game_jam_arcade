...
# Assumes a global variable $chingu_window having the Gosu::Window instance.
...
module Gosu
  class Image
    ...
    def self.autoload(name)
      ret = (path = find_file(name)) ? Gosu::Image.new($chingu_window, path, false) : nil
  ...
  class Font
    ...
    def self.autoload(name, size)
      ...
      return Gosu::Font.new($chingu_window, font_name, size)
    end
