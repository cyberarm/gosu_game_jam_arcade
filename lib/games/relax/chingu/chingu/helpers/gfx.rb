...
module Chingu
  ...
  # All drawings depend on the global variable $chingu_window which should be an instance of Gosu::Window or Chingu::Window
  #
  module GFX
    ...
    #  $chingu_window.draw_quad(0, 0, color,
    #                    $chingu_window.width, 0, color,
    #                    $chingu_window.width, $chingu_window.height, color,
    #                    0, $chingu_window.height, color,
    #                    zorder, :default)
    #end
    #
    ...
    def fill(options, zorder = 0)
      ...
      if options.is_a?(Gosu::Color)
        $chingu_window.draw_quad(0, 0, options,
                        $chingu_window.width, 0, options,
                        $chingu_window.width, $chingu_window.height, options,
                        0, $chingu_window.height, options, zorder, :default)
      else
    ...
    def draw_rect(rect, color, zorder)
      $chingu_window.draw_line(rect.x, rect.y, color, rect.right, rect.y, color, zorder)
      $chingu_window.draw_line(rect.right, rect.y, color, rect.right, rect.bottom, color, zorder)
      $chingu_window.draw_line(rect.right, rect.bottom, color, rect.x, rect.bottom, color, zorder)
      $chingu_window.draw_line(rect.x, rect.bottom, color, rect.x, rect.y, color, zorder)
    end
    ...
    def draw_circle(cx,cy,r,color)
      0.step(360, CIRCLE_STEP) do |a1|
        a2 = a1 + CIRCLE_STEP
        $chingu_window.draw_line cx + Gosu.offset_x(a1, r), cy + Gosu.offset_y(a1, r), color, cx + Gosu.offset_x(a2, r), cy + Gosu.offset_y(a2, r), color, 9999
      end
    end
    ...
    def fill_rect(rect, color, zorder = 0)
      ...
      $chingu_window.draw_quad(  rect.x, rect.y, color,
    ...
    def fill_gradient(options)
      default_options = { :from => Gosu::Color::BLACK,
                          ...
                          :rect => Rect.new([0, 0, $chingu_window.width, $chingu_window.height]),
                          :zorder => 0,
                          :mode => :default
                        }
      ...
      when :vertical
        ...
        colors.each_cons(2) do |from, to|
          $chingu_window.draw_quad(  rect.left,  rect.top,    from,
